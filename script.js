...
async function loadCSV() {
    try {
        ...
        console.log('Nilai Map sample:', allSantri[0]?.nilaiMap);
        return true;
    } catch (error) {
...
function parseSantriLine(line) {
    try {
        ...
        // Keterangan Bolos (parts[5])
        const keteranganBolos = parts[5] || '';
        ...
        // Total Durasi (parts[18])
        const totalDurasi = parts[18] ? parts[18].replace(',', '.') : '0';
        
        // === LIST NILAI (AMBIL DARI PARTS[19]) ===
        // Parts[19]: "4,3,4,4,4,16,4,4,4,4,4,16,4,4,4,4,4,16,4,4,4,4,4,16,4,4,4,4,4,16,"
        // 31 nilai: EH01-EH05(5) + EP1(1) + EH06-EH10(5) + EP2(1) + EH11-EH15(5) + EP3(1) + EH16-EH20(5) + EP4(1) + EH21-EH25(5) + EP5(1) + EA(1) = 31
        const listNilaiStr = parts[19] || '';
        
        // Split, ambil hanya yang angka (abaikan empty string)
        const nilaiArray = listNilaiStr.split(',')
            .map(v => v.trim())
            .filter(v => v !== '' && !isNaN(parseInt(v)))
            .map(v => parseInt(v));
        
        console.log('Nilai array (31 items):', nilaiArray, 'Length:', nilaiArray.length);
        
        // Urutan CSV: EH01-EH05, EP1, EH06-EH10, EP2, EH11-EH15, EP3, EH16-EH20, EP4, EH21-EH25, EP5, EA
        const csvOrder = [
        ...
        for (let i = 0; i < csvOrder.length; i++) {
            if (i < nilaiArray.length) {
                nilaiMap[csvOrder[i]] = nilaiArray[i];
            } else {
                nilaiMap[csvOrder[i]] = null;
            }
        }
        ...
        return {
            ...
            sisaSoalEA, sisaEH, sisaEP, sisaEA_bolos, keteranganBolos,
            peringkatSementara, peringkatAkhir: peringkatAkhir || gradeAkhir,
            nilaiSementara, nilaiAkhir,
            skorSementara, skorAkhir, maxSkor,
...
function searchSantri() {
    ...
    const results = allSantri.filter(s => 
        s.nama.toLowerCase().includes(term) || s.nip.toLowerCase().includes(term)
    );
...
function displayResult(s) {
    ...
    const getStatusHTML = (status) => {
        ...
        if (st.includes('lulus') && !st.includes('tidak')) 
            return '<span style="color:#22a06b;font-weight:700;font-size:1.1em;">LULUS ✅</span>';
        if (st.includes('tidak')) 
            return '<span style="color:#d14343;font-weight:700;font-size:1.1em;">TIDAK LULUS ❌</span>';
        if (st.includes('harus') || st.includes('belum')) 
            return '<span style="color:#e5942e;font-weight:700;font-size:1.1em;">HARUS EA ⚠️</span>';
        return `<span style="font-weight:600;">${status}</span>`;
    };
    ...
    const parseBolos = (ket) => {
        if (!ket) return { EH: '0 EH', EP: '0 EP', EA: '0 EA' };
        const parts = ket.split(',').map(p => p.trim());
        let r = { EH: '0 EH', EP: '0 EP', EA: '0 EA' };
        parts.forEach(p => {
            if (p.includes('EH')) r.EH = p;
            if (p.includes('EP')) r.EP = p;
            if (p.includes('EA')) r.EA = p;
        });
        return r;
    };
    const bolosKet = parseBolos(s.keteranganBolos);
        ...
        <div class="nilai-utama">
            ...
            <div class="nilai-box bolos-box">
                ...
                <div class="bolos-detail">
                    <div class="bolos-item"><div class="bolos-label">EH</div><div class="bolos-value ${s.sisaEH===0?'nol':''}">${bolosKet.EH}</div></div>
                    <div class="bolos-item"><div class="bolos-label">EP</div><div class="bolos-value ${s.sisaEP===0?'nol':''}">${bolosKet.EP}</div></div>
                    <div class="bolos-item"><div class="bolos-label">EA</div><div class="bolos-value ${s.sisaEA_bolos===0?'nol':''}">${bolosKet.EA}</div></div>
                </div>
            </div>
        </div>
...
function buildNilaiButtons(nilaiMap) {
    ...
    urutanCSV.forEach(label => {
        ...
        html += `<button class="${cssClass}" ${onclick} data-label="${label}" data-nilai="${nilai !== null ? nilai : ''}">${label}</button>`;
    });
...
function klikNilai(btn, label, nilai) {
    const isShowingValue = btn.classList.contains('showing-value');
    ...
    if (isShowingValue) {
        ...
        if (nilai === 0) {
            btn.classList.add('nilai-0');
        } else if (nilai === 1) {
            btn.classList.add('nilai-1');
        } else if (nilai === 2) {
            btn.classList.add('nilai-2');
        } else if (nilai === 3) {
            btn.classList.add('nilai-3');
        } else if (nilai >= 4) {
            btn.classList.add('nilai-4');
        }
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (ok) console.log('✅ Siap - 31 nilai per santri');
});