...
// Random cache buster setiap request
const cacheBuster = '&t=' + Math.random() + Date.now();
const response = await fetch(GOOGLE_SHEETS_URL + cacheBuster, {    cache: 'no-cache',
    headers: {
...
function parseSantriLine(line) {
    try {
        ...
        return { peringkat, nama, nip, title, gradeAkhir, status, sisaSoalEA, keteranganEH, keteranganEP, keteranganEA, peringkatSementara, peringkatAkhir: peringkatAkhir || gradeAkhir, nilaiSementara, nilaiAkhir, skorSementara, skorAkhir, maxSkor, totalDurasi, nilaiMap, finalRemark };
